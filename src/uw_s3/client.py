...
class UWS3:
    ...
    def copy_prefix(self, bucket: str, src_prefix: str, dst_prefix: str) -> int:
        ...
        objects = list(
            self.client.list_objects(bucket, prefix=src_prefix, recursive=True)
        )
        count = 0
        for obj in objects:
            name = obj.object_name
            if not name.startswith(src_prefix):
                continue
            new_key = dst_prefix + name[len(src_prefix) :]
            self.client.copy_object(bucket, new_key, CopySource(bucket, name))
            count += 1
        return count
